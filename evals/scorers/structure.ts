...
// `expectedCharacteristics`. Looks for "N type" patterns in the expected strings
...
// Brittle regex lookup.
// `4 boxes` gets parsed as expectedCount === { "rectangle": 4 }
// `4 service boxes`  gets parsed as expectedCount = { }
...
> = ({ output, expected }) => {
  // same as the schema scorer for empty elements
  ...
  // Should probably return null. This indicates missing `expectedCharacteristics`
  ...
  // Should probably return null. This indicates a parsing error on the passed
  // `expectedCharacteristics` from the golden dataset
